...
class ScratchLinearRegression():
    ...
    def __init__(self, num_iter=5000, lr=1e-6, no_bias=False, verbose=False):
        # ハイパーパラメータを属性として記録
        self.iter = num_iter
        self.lr = lr
    ...
    def fit(self, X, y, X_val=np.empty(1), y_val=np.empty(1)):
        ...
        y_val : 次の形のndarray, shape (n_samples, )
            検証データの正解値
        """
        self.y = y[:,None]  #_linear_hypothesisの返り値（行列）にサイズを合わせるためreshapeする
        self.y_val = y_val
        if self.no_bias == False:  #バイアス項がある時
            if X.ndim == 1:  #１次元の時(２次元であればここはスルー)
                X = X[:,None]  #バイアス項と結合できるようにXを２次元に変形
            self.X = np.concatenate([np.ones(X.shape[0])[:,None],X],axis=1)  #バイアス項を結合
            if(len(X_val) != 1) & (len(y_val) != 1): #検証データが入力された場合（入力されない場合は何もしない）
                if X_val.ndim == 1:  #検証データが１次元の時
                    X_val = X_val[:,None]  #バイアス項と結合できるようにXを２次元に変形
                self.X_val = np.concatenate([np.ones(X_val.shape[0])[:,None],X_val],axis=1)  #バイアス項を結合
        ...
        if (len(X_val) != 1) & (len(y_val) != 1):  #検証データが入力された場合
            ...
            for i in range(self.iter-1):  #iter-1回_gradient_descentを実行する
                ...
                if self.verbose:
                    ...
                    print("精度（平均2乗誤差）:{}\n".format(mean_squared_error(self.predict(self.X_val),self.y_val)))
